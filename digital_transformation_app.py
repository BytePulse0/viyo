...
def main():
    ...
    # 获取唯一的股票代码和对应的企业名称
    stock_company_map = data[[stock_code_col, company_name_col]].drop_duplicates().set_index(stock_code_col)['企业名称'].to_dict()
    ...
    # 获取该股票可用的年份
    available_years = sorted(data[data[stock_code_col] == selected_stock][year_col].unique())
    ...
    filtered_data = data[
        ...
        (data[year_col].isin(selected_years))
    ...
    if filtered_data.empty:
        ...
        if '应用维度' in filtered_data.columns:
            y_columns.append('应用维度')
        if '技术维度' in filtered_data.columns:
            y_columns.append('技术维度')
